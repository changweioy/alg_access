...
// added by zhou 2015.7.27
// flag is return value of function
// Active is what function
// Module is in which Alg Module
#define Assert_Debug(flag, Active, Module)           \
	if (IMP_FAILURE == (flag))                       \
    {                                                \
	   printf("%s failure in %s\n", Active, Module); \
	   return IMP_FAILURE;                           \
	}


#define ERR_MESSAGE(err_message) { printf("%s!!!!!!!\n\n\n", err_message); }
...
{
	IMP_FLOAT f32X1;  /**< 左上角x坐标 */
	IMP_FLOAT f32Y1;  /**< 左上角y坐标 */
	IMP_FLOAT f32X2;  /**< 右下角x坐标 */
	IMP_FLOAT f32Y2;  /**< 右下角y坐标 */
} IMP_RECT32F_S;
...
typedef enum impIMAGE_FORMAT_E
{
    ...
    IMAGE_FORMAT_IMP_RGB, //RGB,RGB,RGB
    IMAGE_FORMAT_IMP_R_G_B //RRR,GGG,BBB
} IMAGE_FORMAT_E;
...
{
	IMP_RECT_S stRect;	//changed by mzhang 
	IMP_S32 s32Time;	//changed by mzhang
...
{
	IMP_U32	u32Id;							/**< ID */
	IMP_S32 s32Time;	//added by mzhang 20130130
...
typedef struct impCAR_RECT
{
	IMP_S32 x1,y1,x2,y2,ox1,ox2,oy1,oy2,used,add;
}IMP_CAR_RECT;

/** 算法所需变量*/
typedef struct impHOG_TYPE
{
	double mag,theta;
}HOG_TYPE;


typedef struct impPARA_IN_ALGO
{
	IMP_S32 s32ImgW,s32ImgH;
	IMP_S32 *pd32lbp,*feature,min_add;
	IMP_DOUBLE *feature_hog;
	IMP_U8 *cell;
	IMP_CAR_RECT car[200];
	IMP_S32 car_num;
	IMAGE3_S *pmark,*remark,*gry;
	IMP_U16 *binmark;
	IMAGE3_S **img;
	IMP_DOUBLE *resize_x_rate,*resize_y_rate;
	IMP_FLOAT **mark;
//	IMAGE3_S **mark;
	IMP_U32 scale_num;
	IMP_U32 xpart,ypart,xstep,ystep,draw_x,draw_y,core_x,core_y;
	IMP_S8 svmname[256];
	IMP_U32 jump[256];
	IMP_U32 hog_xpart,hog_ypart;
	IMP_S8 svmfilename[256];
	IMP_U32 win_x;
	IMP_U32 win_y;
	IMP_S32 *s32MinSize;	//连通区域最小值
	IMP_FLOAT *c;	//二值化阈值
	IMP_DOUBLE *IMP_SVMload;
	IMP_DOUBLE *shift,*svmscale;
	HOG_TYPE *hoginfo;
}PARA_IN_ALGO;

typedef struct impRect
{
	IMP_S32 x,y,width,height;
}IMP_Rect;

typedef struct impHAND_TYPE
{
	IMP_S32 u32Type[5];
	PARA_IN_ALGO *(para[5]);
	IMP_S32 s32Wanted;
